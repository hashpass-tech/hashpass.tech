...
1. **Schema + RLS**: `db/migrations/V063__support_system.sql`. Tables are RLS-enabled with no public policies (service-role-route-only access), following the `user_schedule_shares` precedent (`V058`) rather than the `auth.uid()`-keyed pattern used elsewhere in this codebase -- support visitors are not Supabase auth users.
