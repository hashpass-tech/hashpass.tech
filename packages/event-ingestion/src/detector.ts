...
export function inspectPublicHtml(html: string, baseUrl: string, robots = ""): PublicSourceSignals {
  ...
  const links = [...html.matchAll(/<(?:link|a)[^>]+(?:href)=["']([^"']+)["'][^>]*>/gi)].map(m => m[1]);
  ...
  return {
    ...
    apiCandidates: [...new Set([...html.matchAll(/["'](\/(?:api|graphql)\/[^"']*)["']/gi)].map(m => absolute(m[1], baseUrl)))],
    assets: [...new Set([...html.matchAll(/<(?:img|meta)[^>]+(?:src|content)=["']([^"']+)["']/gi)].map(m => m[1]).filter(x => /\.(?:png|jpe?g|webp|svg)(?:$|\?)/i.test(x)).map(x => absolute(x, baseUrl)).concat(scripts.map(x => absolute(x, baseUrl))))],
  };
}
